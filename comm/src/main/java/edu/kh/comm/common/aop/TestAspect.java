...
@Component // 런타임(실행한다) 시 필요한 위치에 코드를 알아서 동적 참여 시킬수 있도록 bean 등록
...
public class TestAspect {
	...
	// *  : 모든 | 아무 값
	// .. : 하위 | 아래 (하위 패키지) | 0개 이상의 매개변수( == 매개변수 자리에 .. 찍혔을때) 
	
	// @Before(포인트컷 작성법) : Pointcut에 지정된 메서드가 수행되기 전 advice 수행을 명시하는 어노테이션
	...
	// @After(포인트컷 작성법) : Pointcut에 지정된 메서드가 수행된 후 advice 수행을 명시하는 어노테이션
