...
@Service // 비지니스 로직 (데이터 가공, DB 연결 등)을 처리하는 객체임을 명시 + Bean 등록  (빈임!)
public class MemberServiceImpl implements MemberService {
	...
	// 	  어쩔 수 없이 Connection이 Service에서 얻어올 수 밖에 없었다.
	...
	public Member login(Member inputMember) {
		...
		// 전달 받은 비밀번호를 암호화하여 
		// DB에서 조회한 비밀번호와 비교(DB에서 비교X == Service에서 비교하겠다)
		...
		// A 회원 / 비밀번호 1234 -> 암호화: abcd
		// B 회원 / 비밀번호 1234 -> 암호화: !@df
		// 같은 비밀번호를 입력해도 암호화는 다르게 나온다
		
		// * 매번 암호화 되는 비밀번호가 달라져서 DB에서 직접 비교 불가능 
		//   대신 Bcrypt 암호화를 지원하는 객체가
		//	 이를 비교하는 기능(메서드) 가지고 있어 이를 활용하면 된다.
		
		// ** Bcrypt 암호화를 사용하기 위해 이를 지원하는 Spring-security 모듈 추가 **
			   // 원래 비밀번호(입력받은 pw평문)         /     암호화된 비밀번호 (DB에서 조회한 비밀번호)
		...
		//loginMember != null :일치하는 이메일을 가진 회원 정보가 있을 경우
