    ...
    $.ajax({
        ...
        success : function(rList){
            ...
            for(let reply of rList){
                ...
                // 중괄호가 생략되었다면 if문에 옆에 오는 한줄에 ; 까지만 해석한다.
                // 작성자
                ...
                if(loginMemberNo != ""){ // 빈칸이면 로그인이 안된 상태  ${}은 값이 null이면 "" 빈칸으로 해석된다  
...
function showInsertReply(parentReplyNo, btn ){
        ...
        if(temp.length>0){ // 답글 작성 textarea가 이미 화면에 존재하는 경우
            ...
            if(confirm("다른 답글을 작성 중입니다. 현재 댓글에 답글을 작성 하시겠습니까?")){
                ...
                temp[0].nextElementSibling.remove() // 버튼영역 삭제 [ 형제 요소 ]
                temp[0].remove(); // textarea 삭제 (기준점은 마지막에 삭제 해야된다!)
                
            } else{
                return; // 함수를 종료 시켜 답글이 생성되지 않게함.
            }
        ...
        // 답글을 작성할 textarea 요소를 생성            
        const textarea = document.createElement("textarea"); 
        
        textarea.classList.add("replyInsertContent");


        // 답글 버튼의 부모의 뒤쪽에 textarea 추가 [답글 버튼을 누르면 그 밑에 textarea가 만들어지는 코드]
        // after(요소) : 뒤쪽에 추가 
        ...
        insertBtn.innerText="등록";                        //(부모댓글 번호, this)     
        ...
        // 답글 버튼 영역을 화면에 추가된 textarea 아래쪽에 버튼을 추가 [등록 /취소 버튼을 추가하겠다는 코드]
...
function insertCancel(btn){
    ...
    btn.parentElement.previousElementSibling.remove(); // 취소의 부모의 이전요소(textarea)제거 
    btn.parentElement.remove(); // 취소의 부모요소(reply-btn-area)제거               
}
...
function insertChildReply(parentReplyNo,btn){
    ...
    // "{K:V, K:V, K:V}" => 단순 문자열 JSON(javascript 문자열) K 값은 무조건 문자열"""
